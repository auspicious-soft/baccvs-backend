 ...
 */
export const createResellListingService = async (req: Request, res: Response) => {
 ...
 */
export const getResellListingByIdService = async (req: Request, res: Response) => {
  const { id } = req.params;
 ...
 */
export const getUserResellListingsService = async (req: Request, res: Response) => {
  const { id: userId } = req.user as JwtPayload;
  const { status } = req.query;
    ...
    const resellListings = await resellModel.find(query)
      .populate({
        path: 'originalPurchase',
        populate: {
          path: 'event'
...
/**
 * Get all available resell listings
 */
export const getAllResellListingsService = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (eventId) {
      ...
      const eventPurchases = await mongoose.model('purchase').find({ event: eventId });
    ...
    console.error("Error fetching all resell listings:", error);
    return errorResponseHandler(
      "Failed to fetch resell listings",
 ...
 */
export const updateResellListingService = async (req: Request, res: Response) => {
  const { id: userId } = req.user as JwtPayload;
  const { id } = req.params;
 ...
 */
export const cancelResellListingService = async (req: Request, res: Response) => {
  const { id: userId } = req.user as JwtPayload;
  const { id } = req.params;
