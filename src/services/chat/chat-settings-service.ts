...
export const updateCommunityConversationBackgroundService = async (req: any, res: Response) => {
  ...
  try {
    ...
    if (!communityConversation) {
      const error = new Error("Community conversation not found") as any;
      error.statusCode = httpStatusCode.NOT_FOUND;
      throw error;
    }
    ...
    if (!community) {
      const error = new Error("You are not a member of this community") as any;
      error.statusCode = httpStatusCode.FORBIDDEN;
      throw error;
    }
    ...
    if (req.headers["content-type"]?.includes("multipart/form-data")) {
      return new Promise<any>((resolve) => {
          ...
          fileUploadPromise = new Promise<void>((resolveUpload, rejectUpload) => {
            ...
            fileStream.on("end", async () => {
              ...
              try {
                ...
                // Create unique filename for background image
                const uniqueFileName = `community_background_${communityConversationId}_${userId}_${Date.now()}_${filename}`;
                
                const uploadedImageUrl = await uploadStreamToS3Service(
                  readableStream,
                  uniqueFileName,
                  mimeType,
                  email || `community_background_${customAlphabet("0123456789", 5)()}`
                );
        ...
        busboyParser.on("finish", async () => {
          ...
          try {
            ...
            // Get current background settings for this user
            const currentSettings = (communityConversation as any).backgroundSettings.get(userId) || {};
            ...
            console.error("Community background update error:", error);
            handleError(
              (error as Error).message || "Error updating community background",
              httpStatusCode.INTERNAL_SERVER_ERROR
            );
          }
        });
      ...
      try {
        ...
        console.error("JSON community background update error:", error);
        // Add statusCode to error if not present
        if (!error.statusCode) {
          error.statusCode = httpStatusCode.INTERNAL_SERVER_ERROR;
        }
    ...
    console.error("Community conversation background service error:", error);
    if (!error.statusCode) {
      error.statusCode = httpStatusCode.INTERNAL_SERVER_ERROR;
    }
    throw error;
