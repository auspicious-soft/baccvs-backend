...
export const generateMultipleSignedUrls = async (
  ...
  try {
    const signedUrls = await Promise.all(
      files.map(async (file) => {
        ...
        const signedUrl = await generateSignedUrlToUploadOn(
          fileName,
          file.type,
          userEmail
        );
...
export const uploadStreamToS3Service = async (
  ...
  fileType: string,
  userEmail: string,
  isTemporary: boolean = false // Set to true for story media
): Promise<string> => {
  const timestamp = Date.now();
  const imageKey = `user-story/${userEmail}/${fileType}/${timestamp}-${fileName}`;
  ...
  // Add expiration metadata for story media (1 day)
  if (isTemporary) {
    const expirationDate = new Date();
    expirationDate.setDate(expirationDate.getDate() + 1);
    params.Expires = Math.floor(expirationDate.getTime() / 1000);
    params.Metadata = {
      expires: expirationDate.toISOString(),
      type: "temporary-story",
    };
  }
